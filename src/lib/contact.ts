...
export async function sendTelegram(message: string) { const token = env("TELEGRAM_BOT_TOKEN"); const chat = env("TELEGRAM_CHAT_ID"); if (!token || !chat) throw new Error("Telegram is not configured"); const response = await fetch(`https://api.telegram.org/bot${token}/sendMessage`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ chat_id: chat, text: message }) }); if (!response.ok) { let detail = "unknown"; try { const payload = await response.json() as { description?: string }; detail = payload.description || detail; } catch {} console.error("Telegram delivery failed", { status: response.status, detail }); throw new Error("Telegram failed"); } }
